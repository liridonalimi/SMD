...
public class CustomersController : ControllerBase
{
    ...
    public CustomersController(SmdDbContext db) => _db = db;
    ...
    private async Task<IActionResult?> ValidateUniqueAsync(string? codeRaw, string? nameRaw, string? emailRaw, Guid? id)
    {
        ...
        if (await _db.Customers.AnyAsync(x => x.Code == code && x.Id != id))
            return Conflict("Ekziston klient tjeter me kete kod.");

        var email = Normalize(emailRaw);
        if (email is not null && await _db.Customers.AnyAsync(x => x.Email == email && x.Id != id))
            return Conflict("Ekziston klient tjeter me kete email.");
...
public class UpsertPartnerRequest
{
