// src/pages/inbound/InboundList.tsx
import { useEffect, useMemo, useRef, useState } from "react";
import { Link, useNavigate, useSearchParams } from "react-router-dom";
import { confirmInbound, getInboundListSummary, inboundPdfUrl, listInbound } from "../../services/inbound";
import type { DocumentListItem, DocumentStatus, InboundListSummary } from "../../types/documents";
...
export default function InboundList() {
    ...
    const [summary, setSummary] = useState<InboundListSummary | null>(null);
...
useEffect(() => {
  // Kur URL ndryshon (p.sh. back/forward, link i kopjuar), përditëso state-in
  isSyncingFromUrl.current = true;

  const nextQ = sp.get("q") ?? "";
  const nextStatus = parseStatus(sp.get("status"));
  const nextFrom = sp.get("from") ?? undefined;
  const nextTo = sp.get("to") ?? undefined;
  const nextEmptyOnly = parseBool(sp.get("emptyOnly"));
  const nextAttentionOnly = parseBool(sp.get("attentionOnly"));
  const nextPaymentStatus = parsePaymentStatus(sp.get("paymentStatus"));
  const nextPage = parseNum(sp.get("page"), 1);
  const nextSort = sp.get("sort") ?? "createdat_desc";

  setQ(nextQ);
  setStatus(nextStatus);
  setFrom(nextFrom);
  setTo(nextTo);
  setEmptyOnly(nextEmptyOnly);
  setAttentionOnly(nextAttentionOnly);
  setPaymentStatus(nextPaymentStatus);
  setPage(nextPage);
  setSort(nextSort);

  // Lëre flag-un të bjerë pas këtij tick-u
  queueMicrotask(() => {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    () => ({
        ...
        [attentionOnly, emptyOnly, from, paymentStatus, qDebounced, status, to, page, pageSize, sort]
    );
...
useEffect(() => {
    ...
    else next.set("status", String(status));
    ...
    if (emptyOnly) next.set("emptyOnly", "true");
    else next.delete("emptyOnly");

    if (attentionOnly) next.set("attentionOnly", "true");
    else next.delete("attentionOnly");
    ...
    // shmang loop nëse s'ka ndryshim real
    if (next.toString() !== sp.toString()) {
    setSp(next, { replace: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
}, [q, status, from, to, emptyOnly, attentionOnly, paymentStatus, page, pageSize, sort]);
...
useEffect(() => {
    ...
    getInboundListSummary(params, ac.signal)
        .then(setSummary)
        .catch((e: unknown) => {
            if (e instanceof DOMException && e.name === "AbortError") return;
            console.error("Failed to load inbound summary", e);
            setSummary(null);
        });
...
return (
    <div>
        <PageIntro
            title={UI.document.inboundTitle}
            subtitle="Kontrollo dokumentet hyrese, filtro sipas statusit dhe hyr shpejt ne detaje."
            actions={
                <button
                    type="button"
...
<button
    ...
    style={{ padding: "8px 12px", borderRadius: 10, opacity: page <= 1 ? 0.5 : 1 }}>
    {UI.common.prev}
</button>
...
<button
    ...
    style={{ padding: "8px 12px", borderRadius: 10, opacity: page >= totalPages ? 0.5 : 1 }}>
    {UI.common.next}
</button>

</div>
<div style={{ marginTop: 12, display: "grid", gap: 10 }}>
            {/* pamja 1 por items listen
{items.map((item) => (
    <Link
    key={item.id}
    to={`/inbound/${item.id}`}
    state={{ from: `/inbound?${sp.toString()}` }}
    style={{
        textDecoration: "none",
        color: "inherit",
        padding: 12,
    ...
    */ }
    ...
    {items.map((item) => {
        ...
        const canQuickConfirm = normalizedStatus !== undefined
            && canConfirmDocument(normalizedStatus, me?.role)
            && (item.linesCount ?? 0) > 0;
    ...
    */}
        ...
        >
            <button
                ...
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    nav(`/inbound/${item.id}`, { state: { from: `/inbound?${sp.toString()}` } });
                }}
            ...
            <button
                ...
                onClick={async (e) => {
                    ...
                    try {
                        await downloadFile(inboundPdfUrl(item.id), `${item.documentNo}.pdf`);
            ...
            <button
                ...
                onClick={async (e) => {
                    ...
                    try {
                        ...
                        // fallback minimal (nëse clipboard s’lejohet)
                ...
                title="Kopjo numrin e dokumentit"
            >
                <CopyIcon />
    ...
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
        ...
        {item.partnerName ? (
            <div style={metaChipStyle}>
                <span style={{ opacity: 0.68 }}>Furnitori:</span> {item.partnerCode ? `${item.partnerCode} - ` : ""}{item.partnerName}
            </div>
        ) : null}
        {item.reference ? (
            <div style={metaChipStyle}>
    ...
    {warningChips.length > 0 ? (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
            {warningChips.map((chip) => (
                <span key={chip.label} style={warningChipStyle(chip.tone)}>
                    {chip.label}
                </span>
            ))}
        </div>
    ...
    {!isHover ? (
        <div style={{ opacity: 0.6, fontSize: 12, marginTop: 8 }}>{UI.document.clickForDetails} →</div>
    ) : (
        <div style={{ opacity: 0.6, fontSize: 12, marginTop: 8 }}>{UI.document.clickToOpenDocument} →</div>
    )}
